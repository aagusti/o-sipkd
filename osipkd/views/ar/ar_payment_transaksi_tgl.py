...
from osipkd.models.ak import Jurnal, JurnalItem, Sap, RekeningSap, Rekening
from osipkd.models.pemda import (Unit)

from osipkd.views.base_view import BaseViews
from osipkd.tools import BULANS    

class view_ar_payment_item_tgl(BaseViews):
    ########                    
    # List #
    ########    
    @view_config(route_name='ar-payment-transaksi-tgl', renderer='templates/ar-payment-transaksi-tgl/list.pt',
                 permission='read')
    def view_list(self):
    ...
    # Action #
    ##########    
    @view_config(route_name='ar-payment-transaksi-tgl-act', renderer='json',
                 permission='read')
    def ar_payment_item_tgl_act(self):
        ses = self.request.session
        req = self.request
        params = req.params
        url_dict = req.matchdict
        if url_dict['act']=='grid':
            ...
            query = DBSession.query(ARPaymentTransaksi.tahun, ARPaymentTransaksi.tanggal, ARPaymentTransaksi.kode,
                                    ARPaymentTransaksi.nama, func.sum(ARPaymentTransaksi.amount).label('amount'),
                                    literal_column("0").label('posted')).\
                              outerjoin(Rekening).\
                              group_by(ARPaymentTransaksi.tahun, ARPaymentTransaksi.tanggal, ARPaymentTransaksi.kode,
                                    ARPaymentTransaksi.nama).\
                              filter(ARPaymentTransaksi.tahun==ses['tahun'],
                                        ARPaymentTransaksi.bulan==ses['bulan'])
    ...
    @view_config(route_name='ar-payment-transaksi-tgl-post', renderer='templates/ar-payment-transaksi-tgl/posting.pt',
                 permission='posting')
    def view_edit_posting(self):
        ...
        if request.POST:
            if 'jurnal' in request.POST: 
                ...
                jui   = row.id
                rows = DBSession.query(ARPaymentTransaksi.rekening_id.label('rekening_id1'),
                                ...
                                ).filter(ARPaymentTransaksi.id==id_inv,
                                ...
                                ).group_by(ARPaymentTransaksi.rekening_id.label('rekening_id1'),
                                           Sap.nama.label('nama1'),
                                           KegiatanItem.kegiatan_sub_id.label('kegiatan_sub_id1'),
                                           ARPaymentTransaksi.amount.label('nilai1'),
                                           RekeningSap.db_lo_sap_id.label('sap1'),
    ...
    @view_config(route_name='ar-payment-transaksi-tgl-unpost', 
                 renderer='templates/ar-payment-transaksi-tgl/unposting.pt',
                 permission='read') 
